...
//    }
    ...
    public GraphResp buildPaperGraphMultiFeature(String arxivId, List<PaperScoreDto> recs) {
        ...
        // [Optimization] N+1 방지: 추천된 arxivId들을 모아서 한 번에 조회
        List<String> recommendedArxivIds = recs.stream()
                .map(PaperScoreDto::arxivId)
                .filter(id -> !id.equals(arxivId)) // center 제외
                .distinct()
                .toList();

        // DB에서 일괄 조회
        List<PaperInfo> foundPapers = paperInfoRepository.findByArxivIdIn(recommendedArxivIds);
        // Map으로 변환 (arxivId -> PaperInfo)
        java.util.Map<String, PaperInfo> paperInfoMap = foundPapers.stream()
                .collect(java.util.stream.Collectors.toMap(PaperInfo::getArxivId, p -> p, (a, b) -> a));
        ...
        for (PaperScoreDto r : recs) {
            ...
            // 추천 노드: 미리 조회한 Map에서 가져옴
            PaperInfo maybePaper = paperInfoMap.get(r.arxivId());
